...
function get_post_views_post_types() {
	...
	$post_views_post_types = array( 'post', $dp_options['photo_slug'], 'page' );
 ...
 * アクセス数メタボックス保存
 */
function save_views_meta_box( $post_id, $post = null ) {
	// check autosave
	...
	if ( ! empty( $post->post_type ) && in_array( $post->post_type, get_post_views_post_types() ) && '' === get_post_meta( $post_id, '_views', true ) ) {
...
add_action( 'save_post', 'save_views_meta_box', 10, 2 );
...
function views_wp_footer() {
	if ( is_preview() || get_tcd_membership_memberpage_type() ) {
		return;
	}
...
function ajax_views_count_up() {
	...
	if ( isset( $_POST['post_id'], $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'views_count_up' ) ) {
		...
		if ( 0 < $post_id && in_array( get_post_type( $post_id ), get_post_views_post_types() ) && in_array( get_post_status( $post_id ), array( 'publish', 'private' ) ) ) {
			if ( USE_VIEWS_RANGE_TABLE ) {
