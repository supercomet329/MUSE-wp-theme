...
/**
 * 投稿された画像を定期実行するためのバッチ
 * 
 * 
 * @return void
 */
function cronPublishTwitter()
{
    global $wpdb;
    publishLog('FILE  => ' . __FILE__ . ' LINE  => ' . __LINE__ . ' START  => Twitter投稿');

    // DBからpublishになっている画像投稿をランダムで一件取得
    $sql = '
        SELECT
            ID
            ,post_title
...
add_action('muse_publish_twitter_cron', 'cronPublishTwitter');

// cron登録処理
if (!wp_next_scheduled('muse_publish_twitter_cron')) {
    date_default_timezone_set('Asia/Tokyo');  // タイムゾーンの設定
    $dateClass = new DateTime();
    $dateClass->setTimezone(new DateTimeZone('Asia/Tokyo'));
    $dateClass->modify('+1 hour');
    wp_schedule_event($dateClass->format('U'), 'hour', 'muse_publish_twitter_cron');
}
